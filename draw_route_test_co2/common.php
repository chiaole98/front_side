...
function save_search_results()
{
    if (isset($_REQUEST["travel_mode"])
        ...
        isset($_REQUEST["cal_co2"]) && isset($_REQUEST["duration_text"]) 
        ...
        $cal_co2 = $_REQUEST["calculate_co2"];
        $origin = $_REQUEST["origin"];
        $destination = $_REQUEST["destination"];
        $duration_text = $_REQUEST["duration_text"];
        $values = "('$username','$travel_mode','$in_kilo','$cal_co2','$origin',
                        '$destination','$duration_text')";
        $sql = "INSERT INTO search_results(
                        username,travel_model,distance_in_kilo,calculate_co2,origin,
